...
<section id="contact" class="py-16 px-4 sm:px-6 lg:px-8 bg-dark-200 contact-section" data-lazy-load>
    <div class="container mx-auto max-w-5xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            ...
            ?>
            <a href="<?php echo esc_attr($item['url']); ?>" <?php echo $item['target'] ? 'target="' . esc_attr($item['target']) . '"' : ''; ?> <?php echo $item['rel'] ? 'rel="' . esc_attr($item['rel']) . '"' : ''; ?> class="glass-card p-6 rounded-xl hover:border-blue-500 hover:border-2 hover:bg-blue-500/10 hover:translate-y-[-5px] transition-all duration-300 shadow-md hover:shadow-blue-500/20 animate-fade-in <?php echo esc_attr($delay_class); ?>">
