...
/**
 * Add lazy-load animation observer for below-the-fold content
 */
function dts_add_lazy_load_script() {
    ?>
    <script>
    (function() {
            ...
            var lazyObserver = new IntersectionObserver(function(entries) {
                entries.forEach(function(entry) {
                    if (entry.isIntersecting) {
                        entry.target.classList.add('dts-animate-in');
                        lazyObserver.unobserve(entry.target);
                    }
                });
            }, { rootMargin: '50px 0px' });
            lazyElements.forEach(function(el) { lazyObserver.observe(el); });
        }
    })();
    </script>
    <?php
...
function dts_init_critical_css() {
    ...
    // Hook into wp_head early to inline critical CSS (first paint before stylesheets load)
    add_action('wp_head', 'dts_inline_critical_css', 1);

    // Add resource hints for performance (DNS prefetch, hero image preload)
    add_action('wp_head', 'dts_add_resource_hints', 2);

    // Lazy-load animation observer (lightweight, no CSS loading logic)
    add_action('wp_footer', 'dts_add_lazy_load_script', 20);
}
